...
8. **必须分两步构建**: `turbo.json` 中 `build` 的 `dependsOn` 为空，所有包会并行构建。必须先构建依赖包再构建 opencode，否则 SDK 竞态会导致 `Could not resolve` 错误
9. **不要使用 `--force`**: 会清空所有包的 turbo 缓存，加剧竞态条件
10. **不要挂载 tmpfs 到 dist**: 构建脚本会 `rm -rf dist`，tmpfs 挂载点无法删除
