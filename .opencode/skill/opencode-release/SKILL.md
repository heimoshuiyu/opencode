...
```bash
git log $(git describe --tags --abbrev=0)..HEAD --name-status --pretty=format:"COMMIT:%H%nAUTHOR:%an%nDATE:%ad%nMESSAGE:%s%n---" --numstat
```

**重要**: 不要启动 sub-agents，直接自行分析所有 commits。

分析步骤：
1. 根据上述 git log 输出，按功能领域对 commits 分组（如 Voice、Background jobs、Bash、Web、Agent/Prompt、Tool 等）
2. 对需要深入了解的 commit，直接使用 `git show <commit-hash>` 查看详细代码变更
3. 分析变更的文件和具体代码修改，理解 commit 的目的和改进点
4. 识别解决的问题或添加的功能
...
7. **直接分析**: 代码分析阶段直接自行分析所有 commits，不要启动 sub-agents
