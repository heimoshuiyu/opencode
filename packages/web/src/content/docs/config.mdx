...
Voice input lets you transcribe speech to text directly in the prompt input. Configure the transcription backend through the `voice` option.

Two backends are supported:

- **`lalm`** (default) — Large Audio Language Model transcription via Gemini or OpenAI-compatible audio models
- **`whisper`** — OpenAI Whisper-compatible transcription API
...
- `type` — `"lalm"` or `"whisper"` (default: `"lalm"`)
- `lalm.model` — provider/model ID for LALM transcription (e.g. `google/gemini-2.5-flash`)
- `lalm.system` — custom system prompt for LALM transcription
- `whisper.url` — Whisper-compatible API endpoint
- `whisper.apiKey` — API key for the Whisper endpoint
- `whisper.model` — model name (default: `whisper-1`)
- `whisper.language` — language code hint
- `hot_words` — list of terms to improve accuracy for domain-specific vocabulary
- `context_pairs` — number of recent user/assistant conversation pairs included as context (default: 3, max: 20)
