...
export const JobOutputTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("JobOutputTool.execute")(function* (
      ...
      log.info("Getting job output", { jobId: params.job_id, maxWaitTime: params.max_wait_time, offset: params.offset })
...
function formatJobOutput(job: BackgroundJobManager.Info, output: string, nextOffset: number): string {
  return [
    `Status: ${job.status}`,
    "",
    output,
