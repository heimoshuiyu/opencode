...
export const layer: Layer.Layer<Service, never, ChildProcessSpawner> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cleanupJob = Effect.fn("BackgroundJobManager.cleanupJob")(function* (jobId: string) {
      ...
      if (!job) return
      yield* Effect.logInfo("Cleaning up job", {
        jobId,
    ...
    const finish = Effect.fn("BackgroundJobManager.finish")(function* (
      ...
      yield* Effect.logInfo("Job completed", { jobId, status: result.info.status, exitCode: result.info.exitCode })
    ...
    const register = Effect.fn("BackgroundJobManager.register")(function* (input: {
      ...
      yield* Effect.logInfo(`${input.label} job`, {
        jobId: job.info.id,
        pid: job.info.pid,
...
export const node = LayerNode.make(defaultLayer, [CrossSpawnSpawner.node])
