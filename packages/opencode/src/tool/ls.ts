...
export const ListTool = Tool.define("list", {
  ...
  async execute(params, ctx) {
    ...
    const ignoreGlobs = [...IGNORE_PATTERNS, ...(params.ignore || [])]
    const files = []
    for await (const file of Ripgrep.files({ cwd: searchPath, hidden: true })) {
      const shouldIgnore = ignoreGlobs.some((pattern) => {
        const glob = pattern.endsWith("/") ? `${pattern}*` : pattern
        return file.startsWith(glob)
      })
      if (!shouldIgnore) {
        files.push(file)
