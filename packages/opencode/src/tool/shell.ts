...
import { formatJobOutput } from "./job-output"
...
export const ShellTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ShellTool.run")(function* (
      ...
      let outputOffset = 0
      let capturedOutput = ""
      let adoptedInfo: BackgroundJobManager.Info | undefined
      ...
      const code: number | null = yield* Effect.gen(function* () {
        ...
        if (exit.kind === "autobg") {
          autoConverted = true
          yield* Effect.logInfo("Auto-converting to background job", { command: input.command, runtime: AUTO_BACKGROUND_TIMEOUT })
          ...
          capturedOutput = list.map((item) => item.text).join("")
      ...
      if (autoConverted && jobId) {
        const end = tail(capturedOutput, limits.maxLines, limits.maxBytes)
        let content = end.text || "(no output so far)"
        if (end.cut) {
          content = `...output truncated. Use job_output tool with job_id="${jobId}" and offset=0 to see full output...\n\n${content}`
        }
        content += `\n\nCommand automatically converted to background job (ID: ${jobId}). Reason: exceeded ${AUTO_BACKGROUND_TIMEOUT / 1000}s limit.\n\nUse job_output tool with job_id="${jobId}" and offset=${outputOffset} to get new output, or job_kill with job_id="${jobId}" to terminate.`
        return {
          title: "Auto-converted to background job",
          output: formatJobOutput(adoptedInfo!, content, outputOffset),
        ...
        return {
          ...
          execute: (params: Parameters, ctx: Tool.Context) =>
            Effect.gen(function* () {
              ...
              if (background) {
                yield* Effect.logInfo("Starting command in background job", { command: params.command, cwd })
