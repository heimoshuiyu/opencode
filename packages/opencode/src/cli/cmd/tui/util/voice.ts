  ...
  const stop = async () => {
    ...
    try {
      ...
      if (type === "alm" && input.server) {
        ...
        const result = await input.server.transcribe({
          audio: base64,
          mime,
          sessionID: input.sessionID?.(),
          prompt: input.prompt?.(),
        })
        if (result.error || !result.data) throw new Error(extractErrorMessage(result.error) || "Voice transcription failed")
      ...
      console.log("voice transcribe failed", { error: extractErrorMessage(error), provider: type })
