...
import { VoiceConfig as VoiceProviderConfig } from "@/voice/config"
...
const noCommandMessage = "No recording command available (install ffmpeg, sox, or arecord)"
...
export function create(input: {
  ...
  const availability = () => VoiceProviderConfig.status(input.transcription?.())
  ...
  const transcribeBuffer = (buffer: ArrayBuffer, mime: string) => {
    ...
    const type = VoiceProviderConfig.providerType(voice)
