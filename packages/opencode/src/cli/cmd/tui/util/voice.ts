...
export function create(input: {
  ...
  const isEnabled = () => {
    ...
    if (type === "alm") return !!voice?.alm?.model
  ...
  const stop = async () => {
    ...
    console.log("voice recorder bytes", { bytes: buffer.byteLength })
    ...
    try {
      ...
      if (type === "alm" && input.server) {
        const base64 = Buffer.from(buffer!).toString("base64")
        const url = new URL("/voice/transcribe", input.server.url)
        if (input.server.directory) url.searchParams.set("directory", input.server.directory)
        const resp = await input.server.fetch(url, {
          method: "POST",
        ...
        response = Whisper.TranscribeResponse.zod.parse(await resp.json())
      } else {
