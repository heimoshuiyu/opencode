...
Fork 版本使用 `Dockerfile.hmsy`（基于 `debian:trixie`），提供两个变体：

- **slim**（默认）：仅包含 ffmpeg 和 ripgrep
- **full**：额外包含 bash、curl、python3、nodejs、npm、glab
...
```bash
# 1. 分别构建两个架构
podman build --arch amd64 -f Dockerfile.hmsy --target full-final -t docker.io/heimoshuiyu/opencode:latest-full-amd64 .
podman build --arch arm64 -f Dockerfile.hmsy --target full-final -t docker.io/heimoshuiyu/opencode:latest-full-arm64 .

# 2. 创建多架构 manifest
podman manifest create docker.io/heimoshuiyu/opencode:latest-full \
  docker.io/heimoshuiyu/opencode:latest-full-amd64 \
  docker.io/heimoshuiyu/opencode:latest-full-arm64

# 3. 推送（必须用 --all）
podman manifest push --all docker.io/heimoshuiyu/opencode:latest-full
```
