...
Fork 版本使用 `Dockerfile.hmsy`（基于 `debian:bookworm`，包含 ffmpeg 和 ripgrep）。

```bash
# 1. 分别构建两个架构
podman build --arch amd64 -f Dockerfile.hmsy -t docker.io/heimoshuiyu/opencode:latest-amd64 .
podman build --arch arm64 -f Dockerfile.hmsy -t docker.io/heimoshuiyu/opencode:latest-arm64 .
