...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const voice = createVoiceInput({
    ...
    promptText: () => {
      const parts: string[] = []
      if (sync.directory) parts.push(`directory: ${sync.directory}`)
      const branch = sync.data.vcs?.branch
      if (branch) parts.push(`branch: ${branch}`)
      const editorText = prompt.current().map((part) => ("content" in part ? part.content : "")).join("")
      const assistant = lastAssistantText()
      return [...parts, assistant, editorText].filter((s) => s.trim()).join("\n")
    },
    addPart,
    editorRef,
    queueScroll,
