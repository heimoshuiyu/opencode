...
export function createVoiceInput(input: VoiceInput) {
  ...
  const [hasLastRecording, setHasLastRecording] = createSignal(false)
  const audio = {
    ...
    chunks: [] as Blob[],
    mime: "",
    lastBlob: undefined as Blob | undefined,
  }
  ...
  const confirmRetry = async () => {
    if (!audio.lastBlob) return
    await transcribeAudio(audio.lastBlob)
  }
